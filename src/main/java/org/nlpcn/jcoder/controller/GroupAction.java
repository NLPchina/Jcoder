...
public class GroupAction {
    ...
    @At
    public Restful add(@Param(value = "hostPorts", array_auto_split = true) String[] hostPorts, @Param("name") String name, @Param(value = "first", df = "true") boolean first) throws Exception {

        if (first && (hostPorts == null || hostPorts.length == 0)) {
            return Restful.fail().msg("group:" + name + " host num is 0");
        }
        ...
        if (!first) {
            ...
            Arrays.stream(hostPorts).filter(StringUtil::isNotBlank).forEach(s -> hostPortsArr.add((String) s));
     ...
     */
    @At
    public Restful flush(@Param("hostPort") String hostPort, @Param("groupName") String groupName, @Param(value = "first", df = "true") boolean first) throws Exception {
